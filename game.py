...
def button_click():
    ...
    i, j = map(int, word_id.split("_"))
    
    # Check if the cell has already been chosen
    if (i, j) in game.chosen_cells:
        flash("Cell already chosen!", "error")
        return render_template("index.html", remaining_words=game.remaining_label, grid=game.grid)
    
    # Add the cell to the chosen cells set
    game.chosen_cells.add((i, j))
    game.word_label += game.grid[i][j]
